...
public class AnnotationParser {
	...
	public static void parsePlacement(OMModification m, Annotation a) {
		...
		List<OMArgument> args = getArgumentsFromModification(m);
		if (args != null) {
			for (OMArgument arg : args) { // transformation,
											// iconTransformation...
				...
				if (m2 != null) {
					...
					if (args2 != null) {
						for (OMArgument arg2 : args2) {
							...
							String argName2 = argName + "."
									+ em2.componentRef.name;
	...
	public static void parseDiagram(OMModification m, Annotation a, String name) {
		a.map.put(name, m);
		List<OMArgument> args = getArgumentsFromModification(m);
		if (args != null) {
			for (OMArgument arg : args) { // coordinateSys, graphics...
	...
	public static OMElementModification getElementModificationFromArgument(
			OMArgument arg) {
	...
	public static OMExpression getExpressionFromElementModification(
			OMElementModification em) {
	...
	public static class GraphicExpression {
		...
		public static List<GraphicExpression> parseGraphic(String text) {
			ModelicaParser parser = new ModelicaParser(new StringReader(text));
			try {
			...
			try {
				...
				while (funcArgs != null) {
					if (funcArgs.expression != null) {
						...
						if (exp.simpleExpression != null
								&& exp.simpleExpression.logicalExpression != null
								&& exp.simpleExpression.logicalExpression.logicalTerms != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.size() > 0
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.size() > 0
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.get(0).relation != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.get(0).relation.arithmeticExpression1 != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.get(0).relation.arithmeticExpression1.term != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.get(0).relation.arithmeticExpression1.term.factor != null
								&& exp.simpleExpression.logicalExpression.logicalTerms
										.get(0).logicalFactors.get(0).relation.arithmeticExpression1.term.factor.primary != null) {
							OMPrimary p = exp.simpleExpression.logicalExpression.logicalTerms
									.get(0).logicalFactors.get(0).relation.arithmeticExpression1.term.factor.primary;
							if (p.funcName != null
									&& p.funcCallArgs != null
									&& p.funcCallArgs.funcArgs != null
									&& p.funcCallArgs.funcArgs.namedArguments != null) {
								res
										.add(new GraphicExpression(
												p.funcName,
												p.funcCallArgs.funcArgs.namedArguments));
							}
						} else {
							System.err
									.println("unknown annotation format for graphics: "
											+ exp.toCode());
						}
					}
				...
				if (res.size() == 0) {
					System.out
							.println("warning: no arguments in annoation for graphics: "
									+ primary.toCode());
				}
				return res;
